 ...
 * @param byte The data to write
 */
void SoftwareI2C::randomWrite(char device_address, char start_address, char byte) {
	device_address = device_address & 0xFE;
	start();
	putByte(device_address);
	getAck();
 ...
 */
void SoftwareI2C::randomWrite(char device_address, char start_address, char* data, char data_bytes) {
	if (data == 0 || data_bytes == 0) return;
 ...
 * @return MSB 32bit unsigned integer
 */
uint32_t SoftwareI2C::read32(char device_address, char start_address) {
	char value_array[4] = {0, 0, 0, 0};
