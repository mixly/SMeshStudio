...
//  Serial.println ("Initializing the devices ...");
  ...
  /** Display 8 uncalibrated light sensor values */  
  uncal = lsa.getUncalibrated();
  for (i=0; i< 8; i++)
  {    
    sprintf (str, "LSArray: sensor array: %d = %d", i, uncal[i] );
    Serial.println(str);
  }
  
  /** Calibrate Black */
  Serial.println ("Place Sensor on Black");

  Serial.println ("Press  LEFT button to continue");
  nxshield.waitForButtonPress(BTN_LEFT);
  lsa.calibrateBlack();
  /** Display 8 light sensor black values */  
  ...
  {    
   ...
   /** Calibrate White */
  Serial.println ("Place Sensor on White");
  Serial.println ("Press  RIGHT button to continue");
  nxshield.waitForButtonPress(BTN_RIGHT);
  lsa.calibrateWhite();
  /** Display 8 light sensor black values */  
  cal = lsa.getCalibrated();
  for (i=0; i< 8; i++)
