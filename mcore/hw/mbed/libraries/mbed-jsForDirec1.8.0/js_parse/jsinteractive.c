...
void jsiAppendHardwareInitialisation(JsVar *str, bool addCallbacks) {
	...
	for (i=0;i<USARTS;i++)   // EV_SERIAL0--EV_SERIAL8
		jsiAppendSerialInitialisation(str, jshGetDeviceString(EV_SERIAL0+i), addCallbacks);
	for (i=0;i<SPISNUM;i++)   //EV_SPI0 - EV_SPI3
		jsiAppendDeviceInitialisation(str, jshGetDeviceString(EV_SPI0+i));
	for (i=0;i<I2CSNUM;i++)   //EV_I2C0 - EV_I2C3
		jsiAppendDeviceInitialisation(str, jshGetDeviceString(EV_I2C0+i));
	// pins
	Pin pin;
...
void jsiIdle() {
	...
	while (maxEvents-- && jshPopIOEvent(&event)) {
		...
		if((int)event.flags < (int)0){
			isWatchingPin = ((~event.flags + 1) & (~ (1 << 30))) - EV_EXTI;
			event.flags = (~event.flags + 1) - isWatchingPin;
		}
	...
	if (jshGetEventsUsed() < IOBUFFER_XON) {
		...
		jshSetFlowControlXON(EV_STDIOSERIAL, true);
		int i;
		for (i=0;i<USARTS;i++)
			jshSetFlowControlXON(EV_SERIAL0+i, true);
	}
	...
	while (jsvObjectIteratorHasValue(&it) && !(jsiStatus & JSIS_TIMERS_CHANGED)) {
		...
		int64_t timeUntilNext = timerTime - timePassed;
		if (timeUntilNext<=0) {
