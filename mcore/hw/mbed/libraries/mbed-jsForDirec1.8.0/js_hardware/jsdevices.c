...
int jshGetCharToTransmit(IOEventFlags device) {
  if (DEVICE_IS_USART(device)) {
    JshSerialDeviceState *deviceState =  getSerialDeviceStates(device);
...
void jshPushIOWatchEvent(IOEventFlags channel,uint32_t pin) {
  bool state =jshPinGetValue(pin);
  //jsiConsolePrintf("state = %d,channel = %d\n",state,~((channel | (state?EV_EXTI_IS_HIGH:0)) + pin) + 1);
  if (callWatchPinEventCallback(pin,state)) {
	//jsiConsolePrintf("state!!!\n");
 ...
 jshPushIOEvent(~((channel | (state?EV_EXTI_IS_HIGH:0)) + pin) + 1, time);
}
...
const char *jshGetDeviceString(IOEventFlags device) {
...
#endif
  case EV_SERIAL0: return "Serial0";
  ...
  case EV_SERIAL7: return "Serial7";
  case EV_SERIAL8: return "Serial8";

case EV_SPI0: return "SPI0";
//#if SPIS>=1
...
IOEventFlags jshFromDeviceString(const char *device) {
  ...
  if (device[0]=='S') {
    if (device[1]=='e' && device[2]=='r' && device[3]=='i' && device[4]=='a' && device[5]=='l' && device[6]!=0 && device[7]==0) {
      if (device[6]=='0') return EV_SERIAL0;
      ...
      if (device[6]=='7') return EV_SERIAL7;
      if (device[6]=='8') return EV_SERIAL8;
    }
    if (device[1]=='P' && device[2]=='I' && device[3]!=0 && device[4]==0) {
      if (device[3]=='0') return EV_SPI0;			
//#if SPIS>=1
