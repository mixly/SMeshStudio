...
//EnableSleep
bool GSM3ShieldV1AccessProvider::EnableSleep(bool ensleep)
{
	bool resp;
	theGSM3ShieldV1ModemCore.setCommandError(0);
	theGSM3ShieldV1ModemCore.setCommandCounter(1);
	theGSM3ShieldV1ModemCore.openCommand(this,ENABLESLEEP);
	if (ensleep)
		theGSM3ShieldV1ModemCore.genericCommand_rq(PSTR("AT+QSCLK=1"));
	else
		theGSM3ShieldV1ModemCore.genericCommand_rq(PSTR("AT+QSCLK=0"));
