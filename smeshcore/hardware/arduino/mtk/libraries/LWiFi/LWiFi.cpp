...
boolean wifiSSID(void *userData)
{
  LWiFiClass *pThis = (LWiFiClass*)userData;
  VMINT ret = vm_wlan_get_para(VM_WLAN_SET_MODE_STA, (void*)&pThis->m_apParam);
  vm_log_info("wifiSSID ret = %d", ret);
  if (ret != VM_WLAN_RESULT_SUCCESS)
  {
    memset(pThis->m_apParam.ssid, 0, sizeof(pThis->m_apParam.ssid));
  }
  return true;
}
...
{
  LTask.remoteCall(&wifiSSID, this);
